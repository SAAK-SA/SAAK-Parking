...
// Reads config from Vite env. When the keys are absent, callers fall back to a
...
/** SELECT rows. `query` is a PostgREST query string, e.g. "status=eq.active&order=created_at.desc". */
