...
import { findEmployee, getBuilding, getBuildingZones } from '../data/mockData';
      ...
      <PublicLayout showBack backTo={`/building/${building.id}`}>
        ...
        <div className="w-full max-w-md bg-white/10 backdrop-blur-xl border border-white/15 rounded-3xl p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 pr-10
                             ...
                             focus:outline-none focus:border-brand-gold/60 focus:ring-1 focus:ring-brand-gold/30
                             transition-colors"
                  dir="ltr"
                />
              </div>
            ...
            <button type="submit" className="w-full bg-brand-gold hover:bg-brand-gold/90 text-white font-bold py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="min-h-screen bg-[#EEF2F7]">
      ...
      <div className="bg-brand-navy px-6 py-4 flex items-center justify-between shadow-lg">
        ...
        <button onClick={() => { setStep('login'); setEmployee(null); setEmployeeId(''); }} className="text-white/50 hover:text-white text-sm transition-colors">
          تسجيل الخروج
        </button>
      </div>
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="card text-center">
            <div className="w-10 h-10 rounded-xl bg-brand-navy/8 flex items-center justify-center mx-auto mb-3">
              <Hash className="w-5 h-5 text-brand-navy" />
            </div>
            <p className="text-xs text-text-secondary mb-1">رقم الموقف</p>
            <p className="text-2xl font-bold text-brand-navy">{employee!.assignedSlot}</p>
          </div>
          <div className="card text-center">
        ...
        <div>
          ...
          <ParkingMap
            ...
            interactive={false}
            highlightSlot={employee!.assignedSlot}
          />
        </div>
      </div>
    </div>
